        ...
        <>
            <nav className="flex flex-row p-5 justify-between items-center">
                ...
                <div className="md:hidden flex" onClick={() => setMenuOpen(!menuOpen)}>
                    <RxHamburgerMenu className="text-gray-900 hover:text-hero" size={32}/>
                </div>
                {menuOpen && (
                    <div className="absolute top-0 left-0 w-full h-full bg-white text-gray-900 z-50">
                        <HiOutlineX className="text-gray-900 absolute top-7 right-5 hover:text-hero" size={32} onClick={() => setMenuOpen(!menuOpen)} />
